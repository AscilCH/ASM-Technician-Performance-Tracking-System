...
export class TechniciensTableComponent implements OnInit {
  ...
  uploadPhoto(file: File): Promise<string> {
    ...
    return this.authService.uploadPhoto(formData).toPromise()
      .then(response => response.filename)
      .catch(error => {
        console.error('Error uploading photo:', error);
        this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Error uploading photo', life: 3000 });
        throw error;
      });
  }
    ...
    if (this.tech.email && this.tech.first_name && this.tech.last_name) {
      try {
        if (this.selectedFile) {
          ...
          this.tech.photo = filename.split('.')[0]; // Save filename without extension
        }
        ...
        console.error('Error in saveTech:', error);
      }
    } else {
